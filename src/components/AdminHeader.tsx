...
export const AdminHeader: React.FC<AdminHeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-800 bg-[#070e1b]/90 px-4 sm:px-6 backdrop-blur-md">
      <div className="flex items-center gap-3">
        {onToggleSidebar && (
          <button
            type="button"
            onClick={onToggleSidebar}
            className="flex size-9 items-center justify-center rounded-xl border border-slate-800 bg-slate-900/80 text-slate-400 hover:bg-slate-800 hover:text-white transition cursor-pointer"
        ...
        <div className="flex items-center gap-2">
          <h1 className="text-sm sm:text-base font-black text-white">{currentTabTitle}</h1>
          {pendingCount > 0 && (
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center gap-1.5 rounded-xl border border-slate-700 bg-slate-800/80 px-2.5 sm:px-3 py-1.5 text-xs font-bold text-slate-300 hover:text-white transition"
        >
